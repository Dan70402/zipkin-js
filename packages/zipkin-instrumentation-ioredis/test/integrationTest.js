...
function getRedis(tracer) {
  return zipkinClient(tracer, IoRedis, redisConnectionOptions);
}

describe('ioredis interceptor', () => {
  ...
  it('should run ioredis calls with callbacks', done => {
    const ctxImpl = new ExplicitContext();
    const recorder = {
      record: () => {
      }
  ...
  it('should run ioredis calls with Promises', done => {
    const ctxImpl = new ExplicitContext();
    const recorder = {
      record: () => {
      }
